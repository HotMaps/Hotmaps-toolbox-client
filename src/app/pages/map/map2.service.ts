...
export class MapService extends APIService implements OnInit, OnDestroy {
    ...
    retriveMapEvent(): void {
        const self = this;
        this.map.on('click', self.onClickEvent);
        this.map.on('baselayerchange', self.onBaselayerChange);
        ...
        this.map.on('didUpdateLayers', self.onDidUpdateLayers);
    ...
    onZoomEnd() { }
    onDidUpdateLayers(e) {
        if (this.selectionToolService.isLayerInMap() === true) {
            this.selectionToolService.openPopup();
            this.logger.log('MapService/didUpdateLayers-----' + e);
        }
    }
    onBaselayerChange(e: LayersControlEvent) {
        this.logger.log('baselayerchange');
        // in this part we manage the selection scale then we refresh the layers
        const scaleLevel = e.name;
        this.selectionToolService.clearAll(this.map);
        this.selectionScaleService.setScaleValue(scaleLevel);
        this.selectionToolService.setScaleValue(scaleLevel);
        this.layersService.setCurrentNutsLevel(scaleLevel);
        if (this.selectionToolService.isLayerInMap() === true) {
            this.selectionToolService.openPopup();
            this.logger.log('MapService/didUpdateLayers-----' + e);
        }
    ...
    onClickEvent(e: MouseEvent) {
        this.logger.log('MapService/click');
        // check if the selection toul is activate
        this.logger.log('MapService/Scale' + this.selectionScaleService.getScaleValue() );
        if (this.selectionScaleService.getScaleValue() === hectare) {
          if (this.layersService.getIsReadyToShowFeatureInfo() === true) {
            const layer = new L.Rectangle(e.latlng.toBounds(100));
            this.selectionToolService.layerCreatedClick(layer, this.map);
          }
        } else {
            this.selectionToolService.enableNavigationService(this.map);
            this.getNutsGeometryFromNuts(e.latlng, this.selectionScaleService.getScaleValue());
        }
    }
