...
import 'leaflet-draw';
import EDITED = L.Draw.Event.EDITED;
import DRAWSTART = L.Draw.Event.DRAWSTART;
import DELETED = L.Draw.Event.DELETED;
import EDITSTOP = L.Draw.Event.EDITSTOP;
import EDITSTART = L.Draw.Event.EDITSTART;
import CREATED = L.Draw.Event.CREATED;
...
export class MapService extends APIService implements OnInit, OnDestroy {
    ...
    retriveMapEvent(): void {
        ...
        this.map.on(CREATED, (e) => { self.onDrawCreated(self, e)});
        this.map.on(EDITED, () => { self.onDrawEdited(self)});
        this.map.on(DRAWSTART, () => { self.onDrawStart(self)});
        this.map.on(EDITSTART, () => { self.onDrawEditStart(self)});
        this.map.on(EDITSTOP, () => { self.onDrawEditStop(self)});
        this.map.on(DELETED, () => { self.onDrawDeleted(self)});
    }
